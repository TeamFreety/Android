...
public class MyPagePostRecyclerAdapter extends RecyclerView.Adapter<MyPagePostHolder> {
    ...
    public void onBindViewHolder(MyPagePostHolder holder, int position) {
        Glide.with(context).load(myPagePostDataList.get(position).getImageURL()).override(400, 400).thumbnail(0.001f).into(holder.getPostImage());
    ...
    public void updatePostDataList(List<MyPagePostData> myPagePostDataList) {
        this.myPagePostDataList = myPagePostDataList;
        notifyDataSetChanged();
    }
    public List<MyPagePostData> getMyPagePostDataList() {
        return myPagePostDataList;
    }
